...
//                   input_dev_num);
    ...
    for (i = 0; i < input_dev_num; i++)
    {
        ...
        if (strstr(tp_name, "ts") || strstr(tp_name, "Goodix") || strstr(tp_name, "ft5x06"))
        {
...
int evdev_init(lv_disp_drv_t *disp_drv, int rot)
{
    ...
    return evdev_set_file(disp_drv, tp_event);
}
/**
...
int evdev_set_file(lv_disp_drv_t *disp_drv, char *dev_name)
{
    ...
    disp_hor = disp_drv->hor_res;
    disp_ver = disp_drv->ver_res;
    if ((evdev_min_x != 0) ||
